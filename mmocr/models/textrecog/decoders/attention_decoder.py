...
class AttenDecoder(BaseDecoder):
    ...
    def forward_train(
        ...
        B = feat.shape[0]
        state = torch.zeros(1, B, self.hidden_size).to(feat.device)
        padded_targets = [
        ...
        for i in range(self.max_seq_len):
            prev_char = padded_targets[:, i].to(feat.device)
            output, state = self._attention(feat, state, prev_char)
